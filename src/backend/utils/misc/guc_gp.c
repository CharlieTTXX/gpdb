...
struct config_bool ConfigureNamesBool_gp[] =
{
...

	{
		{"gp_enable_groupext_dqa_pruning", PGC_USERSET, QUERY_TUNING_METHOD,
			gettext_noop("Planner Only. True: planner will decide whether to use rollup rewrite in the plan for"
			" DQA query based on costs calculation for better performance. False: disable it and back to gather"
			" plan for rollup."),
			NULL,
			GUC_NO_SHOW_ALL | GUC_NOT_IN_SAMPLE
		},
